...
Responsibilities:
  1. Load the compact visible import slice when available, then fall back to
     the scored parquet or historical backfills
...
def build_network_graph(cache_key: str, lang: str = "es") -> dict[str, Any]:
    ...
    # 1. Discover Parquet files. Prefer the compact import slice generated by
    # scripts/import_contracts_to_postgres.py; loading the full scored parquet
    # plus all backfills can exceed CI/serverless memory.
    import_slice_path = os.path.join(DATA_PROCESSED, "contracts_import_slice.parquet")
    if os.path.exists(import_slice_path):
        parquet_files = [import_slice_path]
    else:
        parquet_files = []

    # Fall back to the scored contracts file if no compact slice exists.
    scored_path = os.path.join(DATA_PROCESSED, "scored_contracts.parquet")
    if not parquet_files and os.path.exists(scored_path):
    ...
    if "valor_contrato" not in df.columns and "valor_del_contrato" in df.columns:
        df["valor_contrato"] = df["valor_del_contrato"]
    if "secop_url" not in df.columns and "urlproceso" in df.columns:
        df["secop_url"] = df["urlproceso"]
