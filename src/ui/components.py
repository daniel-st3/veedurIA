...
def render_kpi_row(
    ...
    def _card(title: str, value: str, accent: str = "#007AFF", icon: str = ""):
        return f"""
        <div style="
        ...
        ">
            <p style="margin: 0 0 0.3rem 0; font-size: 0.85rem; color: #8E8E93; font-weight: 500;
                       letter-spacing: 0.02em; text-transform: uppercase;">{icon} {title}</p>
            <h2 style="margin: 0; font-size: 1.8rem; font-weight: 700; color: #111;
                        letter-spacing: -0.02em;">{value}</h2>
        </div>
        """

    with c1:
        st.markdown(_card(t("kpi_total_contracts"), f"{total_contracts:,}", icon="📋"), unsafe_allow_html=True)
    with c2:
        st.markdown(_card(t("kpi_red_flags"), f"{red_flags:,}", accent="#E53935", icon="🔴"), unsafe_allow_html=True)
    with c3:
        ...
        st.markdown(_card(t("kpi_value_at_risk"), formatted, icon="💰"), unsafe_allow_html=True)
    with c4:
        display_entity = top_entity or "—"
        # Truncate long names
        if len(display_entity) > 30:
            display_entity = display_entity[:28] + "…"
        st.markdown(_card(t("kpi_top_entity"), display_entity, icon="🏢"), unsafe_allow_html=True)
