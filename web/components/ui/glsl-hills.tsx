...
// Colombia Andes: deep forest green → vivid hillside green → warm Colombian gold
...
void main(void) {
  ...
  vec3 color = h < 0.5
    ...
    : mix(andesGreen, peakGold, (h - 0.5) * 2.0);

  gl_FragColor = vec4(color, opacity);
}
`;

export function GLSLHills({ speed = 0.35, cameraZ = 118, planeSize = 256, className }: Props) {
  ...
  useEffect(() => {
    ...
    class Plane {
      ...
      tick(delta: number) {
        this.uniforms.time.value += delta * this.animSpeed;
      }
    }
